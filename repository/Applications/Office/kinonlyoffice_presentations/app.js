const TARGET_PATH = '/index.php/apps/onlyoffice/new?name=New%20presentation.pptx&dir=%2F';
const ONLYOFFICE_PATH = '/index.php/apps/onlyoffice/';
const MENU_LOGOUT_COMMAND = 'onlyoffice.presentations.logout';
...
function sendToBridge(type, payload = {}) {
    try {
        ...
        console.error('[kinonlyoffice_presentations] Bridge send failed:', error);
    }
}
...
function handleBridgeMessage(data) {
    switch (data.type) {
        ...
        case 'kinBridgeError':
            loginInProgress = false;
            console.error('[kinonlyoffice_presentations] Bridge error:', data.error, 'action:', data.action);
            break;
    }
}
