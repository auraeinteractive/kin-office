const TARGET_PATH = '/index.php/apps/onlyoffice/new?name=New%20spreadsheet.xlsx&dir=%2F';
const ONLYOFFICE_PATH = '/index.php/apps/onlyoffice/';
const MENU_LOGOUT_COMMAND = 'onlyoffice.spreadsheets.logout';
...
function sendToBridge(type, payload = {}) {
    try {
        ...
        console.error('[kinonlyoffice_spreadsheets] Bridge send failed:', error);
    }
}
...
function handleBridgeMessage(data) {
    switch (data.type) {
        ...
        case 'kinBridgeError':
            loginInProgress = false;
            console.error('[kinonlyoffice_spreadsheets] Bridge error:', data.error, 'action:', data.action);
            break;
    }
}
