...
verify_oidc_discovery_from_nextcloud() {
  ...
  if docker exec nextcloud sh -c 'command -v curl >/dev/null 2>&1'; then
    ...
    # Do not use curl -f: HTTP 404/502 would exit 22 even when TLS/DNS/hairpin are fine.
    # -L follows redirects; require 2xx and OIDC-shaped JSON (issuer key).
    local code
    code="$(docker exec -e "DISCOVERY_URI=${uri}" nextcloud sh -c "curl -kSsSL --max-time 25 -o ${probe} -w '%{http_code}' \"\$DISCOVERY_URI\"")" || code="000"
    ...
    echo "" >&2
    docker exec nextcloud rm -f "${probe}" 2>/dev/null || true
    echo "deploy.sh: Hint: Kin must serve /.well-known/openid-configuration at this URL on the host nginx." >&2
    echo "deploy.sh: If OIDC only exists on :9219, set issuer=https://hostname:9219 in /etc/kin/config.ini so deploy uses that for discovery." >&2
...
# not update existing volumes. Browsers use Host like "<ip>:9219", which must be trusted. LAN/dev
...
# Configure OnlyOffice to use Kin nginx proxy (HTTPS)
ONLYOFFICE_URL="${KIN_OFFICE_PUBLIC_URL}/ds/"
echo "deploy.sh: Configuring OnlyOffice DocumentServerUrl to ${ONLYOFFICE_URL}..."
docker exec --user www-data nextcloud php occ config:app:set onlyoffice DocumentServerUrl --value="${ONLYOFFICE_URL}" 2>/dev/null || true
docker exec --user www-data nextcloud php occ config:app:set onlyoffice DocumentServerInternalUrl --value="http://onlyofficedocs/" 2>/dev/null || true
# Document Server must load/save files and hit callbacks: use internal HTTP to the nextcloud *service* so
# the DS container does not depend on self-signed https://<lan>:9219. verify_peer_off helps PHP→DS HTTPS.
