...
write_kin_nginx_module() {
  ...
  local noop_sw="${ROOT}/nginx/onlyoffice-noop-sw.js"
  if [[ "${DEPLOY_MODE}" -eq 1 ]]; then
    ...
    noop_sw="/opt/kin/modules/kin-office/nginx/onlyoffice-noop-sw.js"
  fi
...
# Regex must win over prefix /ds/ — do not use ^~ on /ds/ or ONLYOFFICE ships a broken service worker.
location ~* ^${prefix}/ds/.*/document_editor_service_worker\.js\$ {
    default_type application/javascript;
...
location ${prefix}/ds/ {
    ...
    sub_filter '<head>' '<head><script>(function(){try{var n=navigator.serviceWorker;if(!n)return;n.getRegistrations().then(function(r){r.forEach(function(x){x.unregister()})});n.register=function(){return Promise.resolve({unregister:function(){return Promise.resolve()},update:function(){return Promise.resolve()}})}}catch(e){}})();</script>';
...
write_system_nginx_module() {
  ...
  local noop_sw="/opt/kin/modules/kin-office/nginx/onlyoffice-noop-sw.js"
